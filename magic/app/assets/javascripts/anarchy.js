...
var anarchy_url = 'http://dev.2kboston.com/wp-content/themes/irrational/js/anarchy_media' // http address for the anarchy-media folder (no trailing slash).
...
var flvwidth = '480' 	// Width of the flv player
var flvheight = '290'	// Height of the flv player (allow 20px for controller)
...
Anarchy.Mp3 = {
	...
	toggle: function(img, url) {
		if (Anarchy.Mp3.playimg == img) Anarchy.Mp3.destroy()
			...
			'width="480" height="40" id="player" align="middle">' +
			...
			'quality="high" wmode="transparent" width="480" height="40" name="player"' +
...
Anarchy.FLV = {
	go: function() {
		var all = document.getElementsByTagName('a')
		for (var i = 0, o; o = all[i]; i++) {
			if(o.href.match(/\.flv$|\.mp4$/i) && o.className!="amplink") {
			...
			'<param name="flashVars" value="controlbar=over"><param name="movie" value="'+anarchy_url+'/flvplayer.swf?click='+anarchy_url+'/images/flvplaybutton.jpg&file='+url+'&showfsbutton='+flvfullscreen+'"> <param name="wmode" value="transparent">' +
...
Anarchy.MOV = {
	...
	go: function() {
		...
		for (var i = 0, o; o = all[i]; i++) {
			if(o.href.match(/\.mov$|\.m4v$|\.m4b$|\.3gp$/i) && o.className!="amplink") {
